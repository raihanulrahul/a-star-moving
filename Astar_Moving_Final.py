...
class AStarPlanner:
    ...
    def obsmap(self,ox,oy):
        ...
        self.obmap = [[False for i in range(self.ywidth)]
                      for i in range(self.xwidth)] #Initially, set the boolean state to be False for all the points in the secondary grid
        for ix in range(self.xwidth):
            x = self.calc_primary_index(ix, self.minx)
            for iy in range(self.ywidth):
                y = self.calc_primary_index(iy, self.miny)
                for iox, ioy in zip(ox, oy):
                    d1 = math.sqrt((iox - x) ** 2 + (ioy - y) ** 2) # d1 is the distance between centers of a point and an obstacle
                    d2=self.rr+wall_padding 
                    # each point of the obstacle has a circular padding (radius=Wall_padding).
                    #d2 Assuming the robot is on the point, calculate d2 which is the sum of robot radius and obstacle radius
                    if d1 <= d2: # If it's True, the robot IS in a dangerous point.
